...
matrix_2 <- matrix(c(7, 8, 9, 10, 11, 12), nrow = 3, ncol = 2)
```
...
curr_date <- as.Date(curr_datetime)
curr_time <- format(curr_datetime, "%H:%M:%S")
```
...
```{r }
# Print the weekday, year, second, and hour
weekday <- weekdays(curr_date)
year <- format(curr_date, "%Y")
second <- format(curr_datetime, "%S")
hour <- format(curr_datetime, "%H")
...
```{r }
# Get the current date and time in GMT-5
curr_datetime_gmt5 <- Sys.time()
```
...
time_difference <- difftime(curr_datetime, curr_datetime_gmt5, units = "secs")

cat("Current Date/Time:", curr_datetime, "\n")
cat("Current Date/Time in GMT-5:", curr_datetime_gmt5, "\n")
cat("Time Difference (seconds):", time_difference, "\n")
```
...
```{r }
# Create a sequence of numbers from 100 to 150
numbers <- 100:150
```
...
half_length <- length(numbers) / 2
second_half <- numbers[(half_length + 1):length(numbers)]
```


```{r }
# Using lapply on the second half
lapply_result <- lapply(list(second_half), calculate_stats)
...
```{r }
# Using sapply on the second half
sapply_result <- sapply(list(second_half), calculate_stats)
# Using tapply on the second half
tapply_result <- tapply(second_half, INDEX = rep(1, length(second_half)), FUN = calculate_stats)
...
# Using split function to break down the dataset on circumference
...
```{r }
# Print values for split_data where circumference is 30 and 75
print("Data for circumference 30:")
print(split_data$`30`)

print("Data for circumference 75:")
print(split_data$`75`)
...
```{r }
# Find the average age of the tree when circumference is 30 and 214
average_age_30 <- mean(split_data$`30`$age)
average_age_214 <- mean(split_data$`214`$age)
